...
Each molecule was represented using an extended-connectivity/Morgan fingerprint with radius 2 and 2048 bits \citep{rogers2010extended}. The same representation was used across all datasets, models, and split protocols. This design choice reduces feature-engineering variability so that the analysis focuses on how split protocols affect benchmark interpretation.

\subsection{Splitting protocols}

Three split protocols were compared.
...
\textbf{Target-balanced scaffold split.} Scaffold groups were selected to preserve train/test scaffold separation while reducing the train/test target mean difference. This protocol is used as a diagnostic counterfactual, not as a universal split standard. It helps test whether a performance gap remains after reducing one major confounder of ordinary scaffold splitting, namely target distribution shift.

The ordinary scaffold split is deterministic because scaffold groups are assigned according to scaffold-group size. Therefore, variation across seeds for this split reflects model-level stochasticity rather than resampling of train/test molecules. In contrast, random and target-balanced scaffold splits include both split-sampling variation and model-level stochasticity.

\subsection{Models and metrics}

For classification tasks, Logistic Regression, Random Forest, and XGBoost were evaluated. Logistic Regression and Random Forest were implemented using scikit-learn \citep{pedregosa2011scikit}; Random Forest is included as a robust nonlinear classical baseline \citep{breiman2001random}. XGBoost was implemented using the XGBoost library \citep{chen2016xgboost}. ROC-AUC was used as the primary classification metric, with accuracy, F1, and average precision retained as supporting metrics. For regression tasks, Ridge Regression, Random Forest, and XGBoost were evaluated. RMSE was used as the primary regression metric, with MAE and $R^2$ retained as supporting metrics.

Fixed hyperparameters were used across all split protocols and random seeds to avoid introducing additional variation from hyperparameter optimization. This design keeps the comparison focused on the effect of the data-splitting protocol.
...
Model scores were interpreted together with split diagnostics. The diagnostics included train/test target mean gaps, number of scaffolds, largest scaffold fraction, singleton scaffold fraction, train/test shared scaffold count, and maximum test-to-train Tanimoto similarity. For each test molecule, the maximum Tanimoto similarity to any training molecule was calculated using Morgan fingerprints. Outlier cases and model-ranking changes were also analyzed to determine whether split choice changes not only absolute performance but also model comparison.
